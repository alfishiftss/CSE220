...
public class HashTable {
    ...
    private int hashFunction( String key ){
       ...
       for( int i=0; i<key.length(); i = i+2){
            res1 = (int)key.charAt(i);
            res2 = (int)key.charAt(i+1);
            String ma = ma+ res1+res2;
            fin = fin+ Integer.parseInt(ma);
       }
       int indx = fin%ht.length;
       return indx;
    }
    ...
    public String searchHashtable( Object[] keyValuePair ){
       String key = (String)keyValuePair[0];
       int val = (int)keyValuePair[1];
       int indx = hashFunction(key);

       PairNode temp = ht[indx];
       while( temp  != null ){
           if(temp.key.equals(key) && temp.value == val){
               return "found";
